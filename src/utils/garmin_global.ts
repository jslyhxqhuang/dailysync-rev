  ...
  for (let i = 0; i < maxRetries; i++) {
    try {
      ...
      if (is429 && i < maxRetries - 1) {
        const wait = baseDelayMs * Math.pow(2, i); // 5s, 10s, 20s
        console.log(`⚠️  ${label} 触发 429 限流，${wait / 1000}s 后重试... (${i + 1}/${maxRetries})`);
...
export const getGaminGlobalClient = async (): Promise<GarminClientType> => {
    ...
    try {
        ...
        if (!currentSession) {
            await withRetry('GarminGlobal login', () => GCClient.login());
            ...
            try {
                ...
                await withRetry('GarminGlobal loadToken', () => GCClient.loadToken(currentSession.oauth1, currentSession.oauth2));
            } catch (e) {
                // 只在登录默认session登录失败，catch到登录错误，需要重新登录时注册sessionChange事件
                console.log('Warn: renew GarminGlobal session..');
                await withRetry('GarminGlobal re-login', () => GCClient.login(GARMIN_GLOBAL_USERNAME, GARMIN_GLOBAL_PASSWORD));
                await updateSessionToDB('GLOBAL', GCClient.sessionJson);

            }

        }
        const userInfo = await withRetry('GarminGlobal getUserProfile', () => GCClient.getUserProfile());
...
export const migrateGarminGlobal2GarminCN = async (count = 200) => {
    ...
    // 从佳明国际区读取活动数据
    const actSlices = await withRetry('GarminGlobal getActivities', () =>
        clientGlobal.getActivities(actIndex, totalAct)
    );
    // only running
    ...
    for (let j = 0; j < runningActs.length; j++) {
        ...
        // 下载佳明原始数据
        const filePath = await withRetry(`downloadGarminActivity ${act.activityId}`, () =>
            downloadGarminActivity(act.activityId, clientGlobal)
        );
        // 上传到佳明中国区
        ...
        await withRetry(`uploadGarminActivity ${act.activityId}`, () =>
            uploadGarminActivity(filePath, clientCn)
        );
        // 等待2秒，避免API请求太过频繁
...
export const syncGarminGlobal2GarminCN = async () => {
    ...
    const cnActs = await withRetry('GarminCN getActivities', () => clientCN.getActivities(0, 1));
    let globalActs = await withRetry('GarminGlobal getActivities', () =>
        clientGlobal.getActivities(0, Number(GARMIN_SYNC_NUM))
    );
    ...
    if (latestCnActStartTime === latestGlobalActStartTime) {
        ...
        for (let i = 0; i < globalActs.length; i++) {
            ...
            if (globalAct.startTimeLocal > latestCnActStartTime) {
                // 下载佳明原始数据
                const filePath = await withRetry(`downloadGarminActivity ${globalAct.activityId}`, () =>
                    downloadGarminActivity(globalAct.activityId, clientGlobal)
                );
                // 上传到佳明中国区的
                ...
                await withRetry(`uploadGarminActivity ${globalAct.activityId}`, () =>
                    uploadGarminActivity(filePath, clientCN)
                );
